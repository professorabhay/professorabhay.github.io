...
class RingBufferWorkletProcessor extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    this._maxChannelCount = options.processorOptions.maxChannelCount || this._channelCount;

    this._inputAudioBuffer =
      new FreeQueue(Module, this._kernelBufferSize, this._channelCount, this._maxChannelCount);
  ...
  process(inputs, outputs, parameters) {
    ...
    this._inputAudioBuffer.pushRing(input);
    ...
    if (this._inputAudioBuffer.framesAvailable >= this._kernelBufferSize) {
      ...
      this._inputAudioBuffer.pullRing(this._inputAudioBuffer.getChannelData());
      ...
      this._outputAudioBuffer.pushRing(this._outputAudioBuffer.getChannelData());
    }

    // Always pull 128 frames out. If the ring buffer does not have enough
    // frames, the output will be silent.
    this._outputAudioBuffer.pullRing(output);
